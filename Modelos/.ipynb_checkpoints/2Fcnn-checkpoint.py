...
# ==============================================================================
# FUNÇÃO PRINCIPAL (CONTRATO COM O main.py)
# ==============================================================================

def executar_modelo_cv(X_treino, y_treino, X_val, epocas, batch_size, learning_rate):
    """
    Define, treina e executa as previsões para o modelo FCNN_1D.
    """
    print("--- Executando Modelo FCNN 1D (PyTorch) ---")
    ...
    y_treino_tensor = torch.from_numpy(y_treino).float().squeeze(-1) # Remove a última dimensão
    ...
    past_frames = X_treino.shape[1]
    future_frames = y_treino.shape[1]
    model = FCNN_1D(past_frames=past_frames, future_frames=future_frames).to(device)
    ...
    for epoch in range(epocas):
        ...
        if (epoch + 1) % 10 == 0:
            print(f"Época [{epoch+1}/{epocas}], Perda: {loss.item():.6f}")
